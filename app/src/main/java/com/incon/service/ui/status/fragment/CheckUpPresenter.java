...
public class CheckUpPresenter extends BasePresenter<CheckUpContract.View> implements
    ...
    @Override
    public void fetchCheckUpServiceRequests(int servicerCenterId, int userId) {
        ...
        DisposableObserver<List<FetchNewRequestResponse>> observer = new DisposableObserver<List<FetchNewRequestResponse>>() {
            ...
            public void onNext(List<FetchNewRequestResponse> fetchNewRequestResponses) {
                getView().loadingCheckUpRequests(fetchNewRequestResponses);
            }
        ...
        if (userId == -1) {
            listObservable = AppApiService.getInstance().fetchCheckupRequestApi(servicerCenterId);
        } else {
            listObservable = AppApiService.getInstance().fetchCheckUpAssignedRequestApi(userId);
        }
