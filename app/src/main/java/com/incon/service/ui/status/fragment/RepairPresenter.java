...
public class RepairPresenter extends BasePresenter<RepairContract.View> implements
    ...
    @Override
    public void fetchRepairServiceRequests(int servicerCenterId, int userId) {
        ...
        DisposableObserver<List<FetchNewRequestResponse>> observer = new DisposableObserver<List<FetchNewRequestResponse>>() {
            ...
            public void onNext(List<FetchNewRequestResponse> fetchNewRequestResponses) {
                getView().loadingRepairServiceRequests(fetchNewRequestResponses);
            }
        ...
        if (userId == -1) {
            listObservable = AppApiService.getInstance().fetchRepairServiceRequestApi(servicerCenterId);
        } else {
            listObservable = AppApiService.getInstance().fetchRepairAssignedRequestApi(userId);
        }
